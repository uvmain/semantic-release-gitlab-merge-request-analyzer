...
function createGitLabApiFetcher(pluginConfig, context) {
  const apiUrl = pluginConfig.api_url ?? context.env.CI_API_V4_URL;

  const token = pluginConfig.token ?? context.env.GITHUB_TOKEN;

  if (!token) {
    throw new Error(
      `Missing required option "token" in plugin config or GITLAB_TOKEN environment variable`
    );
  }
...
export function verifyConditions(pluginConfig, context) {
  ...
  createGitLabApiFetcher(pluginConfig, context);
...
export async function analyzeCommits({ labels }, context) {
  const fetchGitHubApi = createGitLabApiFetcher(labels, context);
...
export async function generateNotes(config, context) {
  const fetchGitHubApi = createGitLabApiFetcher(config, context);
